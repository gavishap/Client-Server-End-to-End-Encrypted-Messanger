...
int aes_example()
{
	...
	hexify2(reinterpret_cast<const unsigned char*>(ciphertext.c_str()), ciphertext.length());	// print binary data nicely
...
int rsa_example()
{
	...
	hexify2(plain, sizeof(plain));		// print binary data nicely
	...
	std::cout << "cipher:" << std::endl;
	hexify2((unsigned char*)cipher.c_str(), cipher.length());	// print binary data nicely
	...
	hexify2((unsigned char*)decrypted.c_str(), decrypted.length());	// print binary data nicely
